...
<div class="w-full mx-auto max-w-7xl px-2 py-5">
    <div class="mx-auto p-6 flex flex-col">
        <div class="font-bold text-3xl mb-5">{{ __('website.Order')}} #{{ $order->order_code }}</div>
                ...
                <div class="shadow-lg p-4 border-2 rounded-lg">
                    <form wire:submit.prevent="submit">
                        ...
                        <div class="p-4 my-4 text-sm text-blue-800 rounded-lg bg-blue-50" role="alert">
                            <span class="font-medium">{{__('website.Promo aplied')}}!</span> 
                            @foreach($order->promos as $promo)
                                <div>{{ $promo->promo->name }} ( -{{ number_format( $promo->promo_amount, 0, ',', '.') }} )</div>
                            @endforeach
                        </div>
                        @endif
                        <div class="font-bold text-xl mt-5 border-b-2">{{ __('website.Total')}} : {{ number_format($order->total_amount, '0', ',', '.') }}</div>
                        ...
                        @if($order->payment_status == null) 
                            <a href="{{ route('customer.process-payment', $order) }}" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5" id="pay-button">{{ __('website.Process Payment')}}</a>
                        @endif
                        </div>
                    </form>
                </div>
