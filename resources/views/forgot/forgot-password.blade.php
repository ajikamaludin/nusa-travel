    ...
    <div class="w-full mx-auto max-w-7xl px-2 py-10">
        <div class="mx-auto max-w-sm p-6 bg-white border border-b-0 border-gray-200 rounded-t-lg shadow">
            <form method="POST" action="{{ route('customer.forgot-password.store') }}">
                @csrf
                <div class="mb-6">
                    <div class="text-2xl font-extrabold">{{ __('website.Forgot Password') }}</div>
                    <div class="text-gray-500">
                        {{ __('website.Forgot password desc')}}
                    </div>
                </div>
                ...
                <button type="submit"
                    class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm w-full sm:w-auto px-5 py-2.5 text-center">{{ __('website.Next')}}</button>
            </form>
        </div>
