<x-modal blur wire:model.lazy="show" spacing="p-0" align="center">
        ...
        <div class="relative bg-white rounded-lg shadow items-center h-full md:h-auto">
            ...
            <div class="flex items-center justify-between p-4 border-b rounded-t">
                ...
                <button type="button" wire:click="toggle"
                    ...
                    <svg wire:loading.remove aria-hidden="true" class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20"
                    ...
                    <div class="flex flex-row justify-between items-center">
                        <div class="flex flex-col">
                            ...
                            <div class="flex flex-row gap-2 text-sm">
                                <p>{{ $trackDepartureChoosed->alternative_name }} </p>
                                <p>|</p>
                    ...
                    <div class="w-full max-w-5xl mx-auto p-2">
                        <div class="flex flex-col" wire:loading.remove>
                            @foreach ($trackDepartures as $track)
                                <livewire:fastboat-item :track="$track" :date="$date" :quantity="$passengers"
                                    :type='1' :wire:key="$track->id.$date" />
                            @endforeach
                        </div>
                    </div>
                    ...
                    <div class="w-full max-w-5xl mx-auto p-2">
                        <div class="flex flex-col" wire:loading.remove>
                            @foreach ($trackReturns as $track)
                                <livewire:fastboat-item :track="$track" :date="$rdate" :quantity="$passengers"
                                    :type='2' :wire:key="$track->id" />
                            @endforeach
                        </div>
                    </div>
