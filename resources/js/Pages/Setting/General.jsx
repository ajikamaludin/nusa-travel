...
export default function General(props) {
    const { data, setData, post, reset, processing, errors } = useForm({
        ...
        whatsapp_float_enable: extractValue(
            props.setting,
            'G_WHATSAPP_FLOAT_ENABLE'
        ),
                ...
                <div className="mx-auto sm:px-6 lg:px-8">
                    <div className="overflow-hidden p-4 shadow-sm sm:rounded-lg bg-white dark:bg-gray-800 flex flex-col">
                        ...
                        <FormFile
                            label={'Slide 1'}
                            onChange={(e) =>
                                setData('slide1', e.target.files[0])
                            }
                            error={errors.slide1}
                            preview={
                                <img
                                    src={`${data.site_slide1}`}
                        ...
                        <FormFile
                            label={'Slide 2'}
                            onChange={(e) =>
                                setData('slide2', e.target.files[0])
                            }
                            error={errors.slide2}
                            preview={
                                <img
                                    src={`${data.site_slide2}`}
                                    className="w-full h-32 object-cover mb-1"
                                    alt="site logo"
                                />
                            }
                        ...
                        <FormFile
                            label={'Slide 3'}
                            onChange={(e) =>
                                setData('slide3', e.target.files[0])
                            }
                            error={errors.slide3}
                            preview={
                                <img
                                    src={`${data.site_slide3}`}
                                    className="w-full h-32 object-cover mb-1"
                                    alt="site logo"
                                />
                            }
