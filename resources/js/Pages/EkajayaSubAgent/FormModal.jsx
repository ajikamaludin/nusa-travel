...
import ProductSelectionInput from '../Ekajaya/SelectionInput'
...
export default function FormModal(props) {
    ...
    const handleSubmit = () => {
        ...
        if (track !== null) {
            put(route('ekajaya-price-agent.update', track), {
                onSuccess: () => handleClose(),
            })
            return
        }
        post(route('ekajaya-price-agent.store'), {
            onSuccess: () => handleClose(),
        })
    }
    ...
    return (
        <Modal
            ...
            toggle={handleClose}
            title={'Harga Agent [API]'}
        >
            <ProductSelectionInput
                label="Track [API]"
                itemSelected={data.fastboat_track_id}
                onItemSelected={(item) => setData('fastboat_track_id', item.id)}
